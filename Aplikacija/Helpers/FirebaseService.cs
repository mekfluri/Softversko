...
public class FirebaseService {
    ...
    public async Task<string[]> UploadMentorRequestPhotos(int predmetId, int userId, IFormFile file1, IFormFile file2){
        ...
        var task1 = new FirebaseStorage(bucket, new FirebaseStorageOptions {
            AuthTokenAsyncFactory = () => user.User.GetIdTokenAsync(),
            ThrowOnCancel = true
        }).Child("mentor-zahtevi").Child($"{predmetId}").Child($"{userId}").Child(file1.Name).PutAsync(firstFile);
        var task2 = new FirebaseStorage(bucket, new FirebaseStorageOptions {
            AuthTokenAsyncFactory = () => user.User.GetIdTokenAsync(),
            ThrowOnCancel = true
        }).Child("mentor-zahtevi").Child($"{predmetId}").Child($"{userId}").Child(file2.Name).PutAsync(secondFile);
        try {
